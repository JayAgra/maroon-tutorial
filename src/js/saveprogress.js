...
function setPageAsRead(pageUID = 0, elementNumber = 0) {
    ...
    document.getElementById("progressIndicator").textContent = Math.round((cookieValue.split(",").map((element) => {return Number(element)}).filter((element) => {return pages.includes(element)}).length / pages.length) * 100)
